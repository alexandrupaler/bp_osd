...
void main_entry()
{
    ...
    const char* argv1 = "sim_scripts/mkmn_input_test.json";
    const char* argv2 = "sim_scripts/output";

    timing simtime;
    cout<<"Start time: "<<simtime.start_time_string<<endl;
    cout<<"Input file: "<<argv1<<endl;
    int elapsed_seconds;
    elapsed_seconds=simtime.elapsed_time_seconds();

    //read in json input file
    ifstream json_input_file(argv1);
    json json_input;
    ...
    output_filename<<argv2<<"/"<<label<<";p:"<<p_label<<";ui:"<<ui<<".json";
...
PYBIND11_MODULE(bp_decode_pybind11, m){
	m.doc() = "pybind11 version of bp_osd_decode";
	m.def("main_entry", &main_entry, "A function with main");
}
